...
/* project_3: Advanced Scheduling */
void
update_nice_recent_cpu(void) {
	struct thread*t;
	struct list_elem * e = list_begin(&all_list);

	int ready = list_size(&ready_list);
	if(thread_current() != idle_thread) ready++;
...
bool thread_priority_comp (const struct list_elem * left, const struct list_elem * right, void *aux){
	struct thread* prev = list_entry(left, struct thread, elem);
	struct thread* next = list_entry(right, struct thread, elem);
